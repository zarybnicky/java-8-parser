...
\par Ve fázi druhé, pak vždy vezmene první dva prvky fronty a seřadíme je. Z těchto dvou prvků vznikne jeden větší, který umístíme na konec fronty. Takto postupujeme dokud nezůstane ve frontě pouze jeden seznam, který je výsledkem řazení. Jelikož naše implementace bere řetězec jako kruhový, může nastat situace kdy se index konečného seznamu, který ukazuje na začátek seřazeného řetězce, není zároveň indexem na začátek řetězce jako celku (Např. index = 2, pro cdab). Z tohoto důvodu je před ukončením funkce řetězec upraven do správné podoby.
